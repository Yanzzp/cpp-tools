...
void MySQLTool::insertData(std::string filePath, std::string fileName, std::string suffix, std::string fileSize) {
    try {
        std::unique_ptr<sql::Statement> stmt(connection->createStatement());
        stmt->execute("INSERT INTO FileName (filePath, fileName, suffix, fileSize) VALUES ('"+filePath+"', '"+fileName+"', '"+suffix+"', '"+fileSize+"')");
    } catch (const sql::SQLException &e) {
...
void MySQLTool::insertFiles(const string &folderPath, bool isRoot) {
        ...
        if (entry.is_regular_file()) {
            ...
            if (isRoot) {
                threads.emplace_back([&, entry]() {
                    ...
                    insertData(filePath, fileName, suffix, fileSize);
            ...
            // 递归调用，但指明不是根目录
            insertFiles(entry.path().string(), false);
        }
    }
